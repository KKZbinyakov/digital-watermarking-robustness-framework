...
cdef void _embed_core(double[:, :] img_view, int blocks_h, int blocks_w,
    ...
    Args:
        ...
        delta: шаг квантования для SVD.
        margin: величина модификации DCT-коэффициентов.
        threshold: порог для DCT.
        block: буфер блока.
...
cdef void _extract_core(double[:, :] img_view, int blocks_h, int blocks_w,
                        ...
                        const double* h, const double* g, int L):
    """
    Ядро извлечения ЦВЗ с голосованием между SVD и DCT уровнями.
...
class DWTSVD_DCT(Ready_Frequency_Embeddings):
    ...
    def embedding(**args):
        """
        Встраивает биты ЦВЗ в сингулярные числа LL-подполосы с последующей
        модификацией DCT-коэффициентов.
        ...
        :param delta: шаг квантования для SVD.
        :param margin: величина модификации DCT-коэффициентов.
        :param threshold: порог для DCT.
    ...
    def extraction(**args):
        """
        Извлекает биты ЦВЗ с голосованием между SVD и DCT уровнями.
